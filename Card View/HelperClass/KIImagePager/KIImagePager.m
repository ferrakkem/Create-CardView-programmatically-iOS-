...
- (void) initialize
{
    ...
    self.captionFont = [UIFont fontWithName:@"SFUIText-Bold" size:14.0f];
...
{
    _captionLabel = [[UILabel alloc] initWithFrame:CGRectMake(10, _scrollView.frame.size.height - 44, _scrollView.frame.size.width - 20, 36)];
    //_captionLabel = [[UILabel alloc] initWithFrame:CGRectMake(11, 3, _scrollView.frame.size.width - 10, 36)];
   ...
   // [_captionLabel setFont:self.captionFont];
    _captionLabel.font = [UIFont fontWithName:@"SFUIText-Bold" size:15.0f];
    //_captionLabel.textColor = [];
...
- (void) loadData
{
    ...
    if([aImageUrls count] > 0) {
        ...
        //blankImage.image = [UIImage imageNamed:@"harrikenLoading.png"];
...
- (void) initializePageControl
{
    ...
    _pageControl.center = CGPointMake(_scrollView.frame.size.width / 2, _scrollView.frame.size.height - 6.0);
...
- (void) updateCaptionLabelForImageAtIndex:(NSUInteger)index
{
    if ([_dataSource respondsToSelector:@selector(captionForImageAtIndex:inPager:)]) {
        if ([[_dataSource captionForImageAtIndex:index inPager:self] length] > 0) {
            ...
            [self setCaptionText:_captionLabel.text];
            return;
        }
    }
...
- (void) setCaptionText:(NSString*)text{
    @try {
        NSArray *arrayWithTwoStrings = [text componentsSeparatedByString:@"\n"];
        NSDictionary *attribs = @{
                                  NSForegroundColorAttributeName:_captionLabel.textColor,
                                  NSFontAttributeName: _captionLabel.font
                                  };
        NSMutableAttributedString *attributedText = [[NSMutableAttributedString alloc] initWithString:_captionLabel.text attributes:attribs];
        [attributedText addAttribute:NSKernAttributeName value:@(0.0) range:NSMakeRange(0, attributedText.length)];
        
        // Green text attributes
        NSRange boldTextRange = [_captionLabel.text rangeOfString:[arrayWithTwoStrings objectAtIndex:1]];
        [attributedText setAttributes:@{NSFontAttributeName:[UIFont fontWithName:@"SFUIText-Regular" size:12.0f]} range:boldTextRange];
        [attributedText addAttribute:NSKernAttributeName value:@(0.0) range:NSMakeRange(0, attributedText.length)];
        
        NSMutableAttributedString* attrStr = [[NSMutableAttributedString alloc] initWithAttributedString:attributedText];
        _captionLabel.attributedText = attrStr;
